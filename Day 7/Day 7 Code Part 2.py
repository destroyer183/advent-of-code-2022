...
def get_needed_space():   
    ...
    for x in Dir.base_dir:

        full_size += int(x.size)

    needed_space = 30000000 - (70000000 - full_size)
    ...
    for x in get_all_keys(Dir.base_dir):
        ...
        if int(x.size) >= needed_space and type(x) == Dir:

            total_size.append(x.size)

    return full_size, needed_space, min(total_size)



full_size, needed_space, total_size = get_needed_space()

print(f"full size: {full_size}\nneeded space: {needed_space}\nsmallest possible directory: {total_size}")